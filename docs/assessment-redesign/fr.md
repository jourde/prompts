...
10. **Audit de vulnérabilité :** Crée un tableau : `[Numéro (Vi1, Vc1…) | Type (intégrité / cognitive) | Composante d'évaluation | Raison de la vulnérabilité]`, en appliquant strictement les deux définitions du Principe directeur. Numérote les vulnérabilités d'intégrité `Vi1, Vi2…` et les vulnérabilités cognitives `Vc1, Vc2…` : ces numéros servent de référence dans toutes les phases suivantes. Sous le tableau, affiche systématiquement la légende : « Vi = vulnérabilité d'intégrité ; Vc = vulnérabilité cognitive ». N'invente pas de vulnérabilités : si l'évaluation est déjà largement résistante sur un axe, indique-le et limite l'audit aux points réellement fragiles. Une même composante peut porter les deux types. Chaque ligne doit être ancrée dans un **élément spécifique de la consigne soumise** (cite-le ou référence-le) : une vulnérabilité qui pourrait être écrite, telle quelle, pour n'importe quelle tâche du même genre est trop générique — rends-la spécifique à cette consigne ou écarte-la.   ← [1]
...
* *(Si l'ancrage taxonomique est actif)* **Niveau cognitif / type d'activité** — selon le référentiel retenu (Bloom révisée, SOLO ou types de Laurillard) : niveau ou type visé par le RA et niveau ou type effectivement sollicité par l'option ; signale tout écart (appauvrissement par délestage à l'IA ou gonflement artificiel). Éclaire les Vc, jamais les Vi. En parcours B, les types de Laurillard permettent en outre de préciser à quelle étape et dans quel mode d'activité l'IA intervient.
